...
| `A` | Automated | Tool-assisted changes: IDE refactoring (rename, extract, move), search & replace, formatters, linters with auto-fix. Deterministic tools only, no AI/LLMs. | Use the intention matching the underlying goal (`R`, `F`, etc.). Prefer `A` when the method is more relevant than the goal, or changes span multiple intentions. |
